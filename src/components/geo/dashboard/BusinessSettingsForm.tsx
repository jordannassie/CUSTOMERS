  ...
  return (
    <form onSubmit={save} className="flex flex-col gap-4">
      ...
      <button
        type="submit"
        disabled={saving}
        className="self-start flex items-center gap-2 bg-[#2563EB] text-white font-bold px-6 py-2.5 rounded-full hover:bg-[#1d4ed8] transition-colors text-sm disabled:opacity-60 mt-2"
      >
...
function FormField({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <label className="block text-xs font-bold text-[#64748B] uppercase tracking-wide mb-1.5">{label}</label>
      {children}
    </div>
  );
}
