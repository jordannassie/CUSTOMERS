...
function Bubble({
  ...
  return (
    <div
      className={`flex flex-col gap-0.5 ${isAI ? "items-end" : "items-start"}`}
      style={{
        opacity: visible ? 1 : 0,
...
const MESSAGES: { role: "caller" | "ai"; text: string; phase: number }[] = [
  {
    role: "caller",
    text: "Hi, I was wondering if you have any appointments available this week?",
    phase: 1,
  },
  {
    role: "ai",
    text: "Absolutely. I can help with that. Are mornings or afternoons better for you?",
    phase: 2,
  },
  { role: "caller", text: "Afternoons.", phase: 3 },
  {
    role: "ai",
    text: "Great. I'll text you our booking link so you can choose the time that works best.",
    phase: 4,
  },
];
  ...
  return (
    <div className="relative select-none">
      ...
      <div
        ...
        style={{
          boxShadow:
            "0 24px 48px rgba(0,0,0,0.12), 0 4px 16px rgba(37,99,235,0.08)",
        }}
      >
        {/* Header bar */}
        <div className="flex items-center gap-2 px-5 py-3 border-b border-gray-100">
          <span
            className="w-2.5 h-2.5 rounded-full bg-[#22C55E] animate-pulse"
            aria-hidden="true"
          />
          <span className="text-xs font-bold text-[#0F172A] tracking-wide">
            Live Call
          </span>
          <span className="ml-auto text-xs font-medium text-[#22C55E]">
            Connected
          </span>
        </div>
        ...
        <div className="px-5 pt-5 pb-4 flex flex-col items-center text-center border-b border-gray-100">
          ...
          <p className="text-sm font-mono text-[#2563EB] mt-1 font-semibold">
            {timer}
          </p>
